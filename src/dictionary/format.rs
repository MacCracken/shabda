...
/// # Errors
///
/// Returns [`ShabdaError::DictParseError`] if the format is invalid or
...
pub fn parse_cmudict(input: &str) -> Result<PronunciationDict> {
    ...
    for (line_num, line) in input.lines().enumerate() {
        ...
        let (word, phonemes_str) = line.split_once("  ").ok_or_else(|| {
            ShabdaError::DictParseError(alloc::format!(
        ...
        if phonemes.is_empty() {
            return Err(ShabdaError::DictParseError(alloc::format!(
...
pub fn to_cmudict(dict: &PronunciationDict) -> String {
    ...
    output.push_str(";;; Generated by shabda\n");
...
/// # Errors
///
/// Returns [`ShabdaError::DictParseError`] if deserialization fails.
#[cfg(feature = "json")]
pub fn from_json(input: &str) -> Result<PronunciationDict> {
    serde_json::from_str(input)
        .map_err(|e| ShabdaError::DictParseError(alloc::format!("JSON parse error: {e}")))
}
...
/// # Errors
///
/// Returns [`ShabdaError::DictParseError`] if serialization fails.
#[cfg(feature = "json")]
pub fn to_json(dict: &PronunciationDict) -> Result<String> {
    serde_json::to_string(dict)
        .map_err(|e| ShabdaError::DictParseError(alloc::format!("JSON serialize error: {e}")))
}
...
/// # Errors
///
/// Returns [`ShabdaError::DictParseError`] on I/O or parse failure.
#[cfg(feature = "std")]
pub fn load_cmudict_file(path: &std::path::Path) -> Result<PronunciationDict> {
    let data = std::fs::read_to_string(path)
        .map_err(|e| ShabdaError::DictParseError(alloc::format!("failed to read file: {e}")))?;
    parse_cmudict(&data)
}
...
/// # Errors
///
/// Returns [`ShabdaError::DictParseError`] on I/O failure.
#[cfg(feature = "std")]
pub fn save_cmudict_file(dict: &PronunciationDict, path: &std::path::Path) -> Result<()> {
    let data = to_cmudict(dict);
    std::fs::write(path, data)
        .map_err(|e| ShabdaError::DictParseError(alloc::format!("failed to write file: {e}")))?;
    Ok(())
}
